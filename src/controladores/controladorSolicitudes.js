...
exports.crearSolicitud = (req, res) => {
    const { mascota_id, usuario_id } = req.body;
    ...
    const sql = `
        INSERT INTO solicitud_adopcion (mascota_id, usuario_id, estado_id, creada_en)
        VALUES (?, ?, 1, NOW())
    `;

    bd.query(sql, [mascota_id, usuario_id], (err, result) => {
        if (err) {
            console.error('Error al crear solicitud:', err);
            return res.status(500).json({ message: 'Error interno al registrar solicitud.' });
        }
...
exports.eliminarSolicitud = (req, res) => {
    ...
    bd.query(sql, [solicitudId], (err, result) => {
        if (err) {
            console.error('Error al eliminar solicitud:', err);
            return res.status(500).json({ message: 'Error interno al eliminar solicitud.' });
        }
